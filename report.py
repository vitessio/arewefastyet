...
def add_tpcc():
    ...
    # Inserting for oltp_qps
    tpcc_qps = "INSERT INTO qps(TPCC_no,total_qps,reads_qps,writes_qps,other_qps) values(%s,%s,%s,%s,%s)"
    mycursor.execute(tpcc_qps,(tpcc_no,data[0]["qps"]["total"],data[0]["qps"]["reads"],data[0]["qps"]["writes"],data[0]["qps"]["other"]))
    conn.commit()
